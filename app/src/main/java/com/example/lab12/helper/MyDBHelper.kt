...
class History(context: Context, name: String = database, factory: SQLiteDatabase.CursorFactory? = null, version: Int = v): SQLiteOpenHelper(context, name, factory, version) {
    companion object {
        private const val database = "database.db" // 資料庫名稱
        private const val v = 2 //資料庫版本
    }

    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("CREATE TABLE myTable(Id text PRIMARY KEY, StartStationName text NOT NULL,EndStationName text NOT NULL)")
        //0.站名 1.經度 2.緯度 3.車站地址 4.車站id
    }
